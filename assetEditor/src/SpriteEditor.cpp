...
{
  if (!s_importPreviewSurface || s_layers.empty())
    return;
  ...
  if (s_importAsNewLayer) {
    ...
    newLayer.surface =
      MakeSurfacePtr(SDL_CreateRGBSurfaceWithFormat(0, s_canvasWidth, s_canvasHeight, 32, SDL_PIXELFORMAT_RGBA32));
...
{
  if (!s_clipboardSurface || s_layers.empty())
    return;
...
static void ResizeCanvas(int newWidth, int newHeight, SpriteInfo *info)
{
  if (newWidth <= 0 || newHeight <= 0)
    return;
  if (newWidth == s_canvasWidth && newHeight == s_canvasHeight)
    return;
...
static void Undo()
{
  if (s_undoStack.empty() || s_activeLayerIndex >= static_cast<int>(s_layers.size()))
    return;
...
static void Redo()
{
  if (s_redoStack.empty() || s_activeLayerIndex >= static_cast<int>(s_layers.size()))
    return;
...
static SDL_Surface *CompositeLayersToSurface()
{
  ...
  std::sort(layerIndices.begin(), layerIndices.end(),
            [](int a, int b) { return s_layers[a].zOrder < s_layers[b].zOrder; });
...
void RefreshSpriteList()
{
  ...
  std::sort(s_spriteCache.begin(), s_spriteCache.end(),
            [](const SpriteInfo &a, const SpriteInfo &b) { return a.filename < b.filename; });
}
      ...
      for (int i = static_cast<int>(s_layers.size()) - 1; i >= 0; i--) {
        ...
        if (i < static_cast<int>(s_layers.size()) - 1) {
          if (ImGui::SmallButton("▲")) {
            ...
            if (s_activeLayerIndex == i)
              s_activeLayerIndex = i + 1;
            else if (s_activeLayerIndex == i + 1)
              s_activeLayerIndex = i;
  ...
  ImGui::BeginChild("Canvas", ImVec2(0, 0), true, ImGuiWindowFlags_HorizontalScrollbar);
  {
    ...
    if (s_showGrid && displayScale >= 2.0f) {
      ...
      for (int x = 0; x <= spriteInfo->width; x++) {
        ...
        drawList->AddLine(ImVec2(posX, canvasPos.y), ImVec2(posX, canvasPos.y + displayH), gridColor);
      }

      // Horizontal lines
      for (int y = 0; y <= spriteInfo->height; y++) {
        float posY = canvasPos.y + y * displayScale;
        drawList->AddLine(ImVec2(canvasPos.x, posY), ImVec2(canvasPos.x + displayW, posY), gridColor);
      }
    }
    ...
    if (isHovered) {
      ...
      if (pixelX >= 0 && pixelX < spriteInfo->width && pixelY >= 0 && pixelY < spriteInfo->height) {
        ...
        if (s_selectedTool < 2) {
          for (int bx = -s_brushSize / 2; bx <= s_brushSize / 2; bx++) {
            for (int by = -s_brushSize / 2; by <= s_brushSize / 2; by++) {
              ...
              if (px >= 0 && px < spriteInfo->width && py >= 0 && py < spriteInfo->height) {
                ...
                drawList->AddRectFilled(ImVec2(rectX, rectY), ImVec2(rectX + displayScale, rectY + displayScale),
                                        previewColor);
              }
            }
          }
          ...
          drawList->AddRect(ImVec2(rectX, rectY), ImVec2(rectX + displayScale, rectY + displayScale),
      ...
      if (ImGui::IsMouseDown(0)) {
        ...
        if (s_isMoving && (s_moveOffsetX != 0 || s_moveOffsetY != 0)) {
          if (s_activeLayerIndex < static_cast<int>(s_layers.size()) && s_layers[s_activeLayerIndex].surface) {
            ...
            SDL_Surface *shiftedSurface =
              SDL_CreateRGBSurfaceWithFormat(0, activeSurface->w, activeSurface->h, 32, SDL_PIXELFORMAT_RGBA32);
            ...
            if (shiftedSurface) {
              ...
              if (SDL_MUSTLOCK(activeSurface))
                SDL_LockSurface(activeSurface);
              if (SDL_MUSTLOCK(shiftedSurface))
                SDL_LockSurface(shiftedSurface);
              ...
              for (int y = 0; y < activeSurface->h; y++) {
                for (int x = 0; x < activeSurface->w; x++) {
                  ...
                  if (newX >= 0 && newX < activeSurface->w && newY >= 0 && newY < activeSurface->h) {
              ...
              if (SDL_MUSTLOCK(activeSurface))
                SDL_UnlockSurface(activeSurface);
              if (SDL_MUSTLOCK(shiftedSurface))
                SDL_UnlockSurface(shiftedSurface);
...
void RenderSpriteEditorUI()
{
  ...
  if (ImGui::BeginPopupModal("Import Image##Modal", &s_showImportOverlay, ImGuiWindowFlags_AlwaysAutoResize)) {
    ...
    if (s_importPreviewSurface) {
      ...
      ImGui::Spacing();
      ImGui::TextWrapped(
        "Please drag & drop an image file onto this window, or select one from the sprite list on the left.");
    }
  ...
  if (s_notificationTimer > 0.0f) {
    ...
    ImGuiWindowFlags notifFlags = ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove |
