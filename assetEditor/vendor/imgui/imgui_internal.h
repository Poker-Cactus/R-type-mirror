...
enum ImGuiInputFlags_ {
  ...
  // In theory ImGuiInputFlags_RepeatUntilOtherKeyPress may be a desirable default, but it would break too many behavior
  ...
  ImGuiInputFlags_LockThisFrame = 1 << 10, // Access to key data will require EXPLICIT owner ID (ImGuiKeyOwner_Any/0
  ...
  ImGuiInputFlags_RouteGlobalHigh = 1 << 15, // Register route globally (highest priority: unlikely you need to use
  ...
  ImGuiInputFlags_SupportedByShortcut =
    ImGuiInputFlags_RepeatMask_ | ImGuiInputFlags_RouteMask_ | ImGuiInputFlags_RouteExtraMask_,
...
enum ImGuiScrollFlags_ {
  ...
  ImGuiScrollFlags_KeepVisibleEdgeY =
    1 << 1, // If item is not visible: scroll as little as possible on Y axis to bring item back into view [default for
            // Y axis for windows that are already visible]
...
enum ImGuiTypingSelectFlags_ {
  ...
  ImGuiTypingSelectFlags_AllowBackspace = 1
    << 0, // Backspace to delete character inputs. If using: ensure GetTypingSelectRequest() is not called more than
...
struct IMGUI_API ImGuiTypingSelectState {
  ...
  bool SingleCharModeLock =
    false; // After a certain single char repeat count we lock into SingleCharMode. Two benefits: 1) buffer never fill,
           // 2) we can provide an immediate SingleChar mode without timer elapsing.
...
enum ImGuiOldColumnFlags_ {
  ...
  ImGuiOldColumnFlags_GrowParentContentsSize = 1
    << 4, // Restore pre-1.51 behavior of extending the parent window contents size but _without affecting the columns
...
struct ImGuiViewportP : public ImGuiViewport {
  ...
  ImDrawList
    *BgFgDrawLists[2]; // Convenience background (0) and foreground (1) draw lists. We use them to draw software mouser
...
struct ImGuiContext {
  ...
  double LastKeyModsChangeFromNoneTime; // Record the last time key mods changed away from being 0 (affect repeat delay
  ...
  ImU32
    ActiveIdUsingNavInputMask; // If you used this. Since (IMGUI_VERSION_NUM >= 18804) : 'g.ActiveIdUsingNavInputMask |=
                               // (1 << ImGuiNavInput_Cancel);' becomes 'SetKeyOwner(ImGuiKey_Escape, g.ActiveId) and/or
