...
IsWindowHovered(ImGuiHoveredFlags flags =
                  0); // is current window hovered and hoverable (e.g. not blocked by a popup/modal)? See
                      ...
                      // be dispatched to Dear ImGui or to your underlying app, you should not use this function! Use
                      // the 'io.WantCaptureMouse' boolean for that! Refer to FAQ entry "How can I tell whether to
...
IMGUI_API void SetNextWindowSizeConstraints(
  ...
  void *custom_callback_data = NULL); // set next window size limits. use 0.0f or FLT_MAX if you don't want limits. Use
                                      // -1 for both min and max of same axis to preserve current size (which itself is
                                      // a constraint). Use callback to apply non-trivial programmatic constraints.
...
IMGUI_API ImU32 GetColorU32(
  ImGuiCol idx, float alpha_mul = 1.0f); // retrieve given style color with style alpha applied and optional extra alpha
...
Selectable(const char *label, bool selected = false, ImGuiSelectableFlags flags = 0,
           const ImVec2 &size =
             ImVec2(0, 0)); // "bool selected" carry the selection state (read-only). Selectable() is clicked is returns
                            // true so you can modify your selection state. size.x==0.0: use remaining width,
...
IMGUI_API void SetTooltip(const char *fmt, ...)
  IM_FMTARGS(1); // set a text-only tooltip. Often used after a ImGui::IsItemHovered() check. Override any previous call
                 // to SetTooltip().
...
IMGUI_API bool SetDragDropPayload(
  ...
  ImGuiCond cond =
    0); // type is a user defined string of maximum 32 characters. Strings starting with '_' are reserved for dear imgui
...
enum ImGuiWindowFlags_ {
  ...
  ImGuiWindowFlags_NavFlattened =
    1 << 23, // [BETA] On child window: share focus scope, allow gamepad/keyboard navigation to cross over parent border
...
enum ImGuiInputTextFlags_ {
  ...
  ImGuiInputTextFlags_EnterReturnsTrue = 1
    << 5, // Return 'true' when Enter is pressed (as opposed to every time the value was modified). Consider looking at
  ...
  ImGuiInputTextFlags_CallbackEdit =
    1 << 19, // Callback on any edit (note that InputText() already returns true on edit, the callback is useful mainly
             // to manipulate the underlying buffer while focus is active)
...
enum ImGuiTabBarFlags_ {
  ...
  ImGuiTabBarFlags_NoCloseWithMiddleMouseButton = 1
    << 3, // Disable behavior of closing tabs (that are submitted with p_open != NULL) with middle mouse button. You may
          // handle this behavior manually on user's side with if (IsItemHovered() && IsMouseClicked(2)) *p_open =
          // false.
...
enum ImGuiHoveredFlags_ {
  ...
  ImGuiHoveredFlags_Stationary = 1
    << 13, // Require mouse to be stationary for style.HoverStationaryDelay (~0.15 sec) _at least one time_. After this,
...
enum ImGuiDragDropFlags_ {
  ...
  ImGuiDragDropFlags_SourceNoDisableHover = 1
    << 1, // By default, when dragging we clear data so that IsItemHovered() will return false, to avoid subsequent user
          // code submitting tooltips. This flag disables this behavior so you can still call IsItemHovered() on the
          // source item.
...
enum ImGuiSliderFlags_ {
  ...
  ImGuiSliderFlags_InvalidMask_ =
    0x7000000F, // [Internal] We treat using those bits as being potentially a 'float power' argument from the previous
...
struct ImGuiIO {
  ...
  ImWchar
    PlatformLocaleDecimalPoint; // '.'              // [Experimental] Configure decimal point e.g. '.' or ',' useful for
  ...
  bool
    WantCaptureMouse; // Set when Dear ImGui will use mouse inputs, in this case do not dispatch them to your main
...
enum ImDrawFlags_ {
  ...
  ImDrawFlags_RoundCornersBottomRight = 1
    << 7, // AddRect(), AddRectFilled(), PathRect(): enable rounding bottom-right corner only (when rounding > 0.0f, we
...
struct ImFontConfig {
  ...
  float
    RasterizerDensity; // 1.0f     // DPI scale for rasterization, not altering other font metrics: make it easy to swap
