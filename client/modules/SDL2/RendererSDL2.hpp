...
/**
 * @class RendererSDL2
 * @brief SDL2-based implementation of the renderer interfaces.
 */
class RendererSDL2 : public IRenderer, public IColorBlindSupport
{
public:
  /** @brief Create an SDL2 renderer with a given window size. */
  ...
  // === Color Blind Filter ===
  /// @copydoc IColorBlindSupport::setColorBlindMode
  void setColorBlindMode(ColorBlindMode mode) override;
  /// @copydoc IColorBlindSupport::getColorBlindMode
...
private:
  ...
  /** @brief Apply the color-blind filter overlay to the render target. */
  void applyColorBlindOverlay();
  /** @brief Initialize the offscreen render target for filtering. */
  void initRenderTarget();
  /** @brief Release the offscreen render target resources. */
