...
namespace {
  ...
  Color getBarColor(float percent) {
    if (percent < 50.0f) return BAR_GREEN;
    if (percent < 80.0f) return BAR_YELLOW;
    return BAR_RED;
  }
...
void InfoMode::updateDynamicSystemInfo()
{
  ...
  if (host_statistics(mach_host_self(), HOST_CPU_LOAD_INFO, 
    ...
    uint64_t total = idle + cpuInfo.cpu_ticks[CPU_STATE_USER] + 
                     cpuInfo.cpu_ticks[CPU_STATE_SYSTEM] + cpuInfo.cpu_ticks[CPU_STATE_NICE];
...
{
  if (!m_isActive || !m_renderer || !m_hudFont) return;
